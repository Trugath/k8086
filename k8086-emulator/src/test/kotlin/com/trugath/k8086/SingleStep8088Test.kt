...
/**
 * State-level runner for SingleStepTests/8088 v2 hardware vectors.
 *
 * Prefetch queue and bus cycles are ignored — same architectural scope as the 8086 runner.
 */
@Tag("single-step-8088")
class SingleStep8088Test {
    ...
    fun hardwareGeneratedInstructionVectors() {
        assumeTrue(
            ...
            "Run with ./gradlew :k8086-emulator:singleStep8088Test",
        )

        val corpus = File("testdata/8088/v2")
        check(corpus.isDirectory) {
            "8088 corpus is missing. Run: git submodule update --init --recursive"
        }
        ...
        if (failures.isNotEmpty()) {
            fail<Nothing>(
                buildString {
                    append("SingleStepTests/8088: ${failures.size} failure(s) after $executed vectors")
                    appendLine()
                    failures.forEach {
                        appendLine()
                        appendLine(it)
    ...
    private fun failureHeader(fileName: String, vector: JsonNode): String =
        "$fileName #${vector["idx"]?.asInt() ?: vector["test_num"]?.asInt() ?: -1} " +
            "${vector["name"]?.asText() ?: "<unnamed>"} " +
            "hash=${vector["hash"]?.asText() ?: vector["test_hash"]?.asText() ?: "<none>"}"
